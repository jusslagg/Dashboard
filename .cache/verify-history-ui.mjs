...
const tabs=await (await fetch('http://127.0.0.1:9242/json/list')).json();
...
try{
 ...
 await send('Page.navigate',{url:'http://127.0.0.1:8009/historial'});
 for(let i=0;i<200;i++){if(await run(`typeof historialActual!=='undefined'&&historialActual.length>0`))break;await new Promise(r=>setTimeout(r,100))}
 const result=await run(`({title:document.querySelector('main h1')?.textContent,events:historialActual.length,undo:document.body.innerText.includes('Deshacer movimiento'),error:document.body.innerText.includes('No se pudo cargar')})`);
 assert.equal(result.title,'Historial de cambios');assert(result.events>0);assert.equal(result.error,false);assert.deepEqual(errors,[]);
