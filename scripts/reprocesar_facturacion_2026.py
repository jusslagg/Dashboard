...
def main(aplicar=False):
    ...
    with app.app_context():
        ...
        for r in registros:
            ...
            r.facturado_horas_manual = numero(row[27])
            r.variable_productivo = numero(row[28])
            ajuste = numero(row[29])
            r.bonos = max(ajuste, 0)
            r.penalizaciones = abs(min(ajuste, 0))
            r.total_facturado_manual = numero(row[30])
        ...
        if aplicar and not sin_match_material:
            ...
            objetivos = defaultdict(lambda: [0,0,0,0,0])
            for row in ws.iter_rows(min_row=3, max_col=31, values_only=True):
                if not row[4] or not row[9]: continue
                grupo = (mes(row[9]), maestro(row[4]))
                for i, columna in enumerate((24,25,27,28,29)):
            ...
            for grupo, deseado in objetivos.items():
                ...
                primero.facturado_horas_manual += deseado[2] - sum(float(r.facturado_horas or 0) for r in propios)
                primero.variable_productivo += deseado[3] - sum(float(r.variable_productivo_calculo or 0) for r in propios)
                ajuste_actual = sum(float(r.facturado_bono or 0) + float(r.penalizaciones_incumplimientos or 0) for r in propios)
                delta_ajuste = deseado[4] - ajuste_actual
                if delta_ajuste >= 0: primero.bonos += delta_ajuste
                else: primero.penalizaciones += abs(delta_ajuste)
